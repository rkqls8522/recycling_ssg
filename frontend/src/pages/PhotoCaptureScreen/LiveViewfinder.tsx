...
const CONFIG = {
  ...
  grabCutIntervalMs: 200, // 워커가 바쁘지 않을 때, 이 간격으로 새 요청을 보냄
...
function drawCorner(ctx: CanvasRenderingContext2D, cx: number, cy: number, dx: number, dy: number, color: string) {
...
const LiveViewfinder = forwardRef<LiveViewfinderHandle, Props>(function LiveViewfinder(
  ...
  const guideBoxToRawRect = useCallback(() => {
    ...
    const preMirrorX = isFrontCameraRef.current ? overlay.width - gx - gw : gx;
  ...
  const guideSampleRectToDisplayBox = useCallback(
    (localRect: { x: number; y: number; width: number; height: number }): DisplayBox => {
      const overlay = overlayRef.current;
      ...
      const displayLocalX = isFrontCameraRef.current ? gw - localX - localW : localX;
  ...
  const maybeRequestGrabCut = useCallback(() => {
    ...
    const gctx = sample.getContext("2d", { willReadFrequently: true } as CanvasRenderingContext2DSettings);
    if (!gctx) return;
    gctx.drawImage(video, r.x, r.y, r.w, r.h, 0, 0, sample.width, sample.height);
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        isFrontCameraRef.current = settings.facingMode ? settings.facingMode === "user" : false;

        const sampleW = 100;
        const sampleH = Math.max(1, Math.round(sampleW * (GUIDE_BOX.h / GUIDE_BOX.w)));
      ...
      const worker = new Worker(new URL("./grabcut.worker.ts", import.meta.url));
  ...
  return (
    <div className="absolute inset-0">
      ...
      {flashOn && <div className="absolute inset-0 bg-white opacity-80 transition-opacity duration-300" />}
