...
| `class_id`       | Integer \| null | Yes      | `AI_LOW_CONFIDENCE` 일 때 모델이 (확신은 낮지만) 예측한 class_id. `AI_NO_MAIN_OBJECT` 는 예측 자체가 없으므로 `null`                                       |
| `major_category` | String \| null  | Yes      | `class_id` 와 같은 대상의 대분류. `class_id` 와 세트로 채워지거나 함께 `null`                                                                              |
| `minor_category` | String \| null  | Yes      | `class_id` 와 같은 대상의 소분류. `class_id` 와 세트로 채워지거나 함께 `null`                                                                              |
...
```json
{
  "status": "ok",
  "model_loaded": true,
  "model_version": "B00_no_aug_control_seed42"
}
```
