...
def test_favorites_crud_flow(client, signup_and_login):
    ...
    created = client.post("/api/v1/favorites", json={"class_id": 77}, headers=headers)
    assert created.status_code == 201
    body = created.json()
    assert body["class_id"] == 77
    assert body["major_category"] == "플라스틱류"
    favorite_id = body["favorite_id"]

    duplicate = client.post("/api/v1/favorites", json={"class_id": 77}, headers=headers)
...
def test_favorites_are_scoped_per_user(client, signup_and_login):
    ...
    created = client.post("/api/v1/favorites", json={"class_id": 22}, headers=headers_a)
